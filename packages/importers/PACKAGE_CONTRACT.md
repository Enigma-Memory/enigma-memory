...
- Import previews use `schema: enigma.import_preview.v1` and expose only counts, refs, commitments, limitations, recommended review/import actions, duplicate groups, a public-safe `primary_action`, and an `enigma.import_preview_receipt.v1`. They never return `memory_candidates.content`.
