...
- `enigma_import_approve` must require `approved:true`, fail closed when review is still required, and return `enigma.import_approved_batch.v1` with batch receipt metadata only.
