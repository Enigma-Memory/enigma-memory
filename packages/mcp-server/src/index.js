...
import { createPassport, compileContextPack, renderContextPackForModel } from '../../passport/src/index.js';
...
import {
  ...
  MCP_TOOL_PROTOCOL_RULES,
  analyzeMcpToolProtocol,
} from './tool-protocol.js';

export { MCP_TOOL_PROTOCOL_AUDIT_SCHEMA, MCP_TOOL_PROTOCOL_RULES };
...
function validateToolArguments(name, args) {
  ...
  switch (name) {
    ...
    case 'enigma_visual_context':
      ...
      optionalBoolean(args, 'optimize', name);
      optionalNonNegativeNumber(args, 'max_estimated_tokens', name);
      optionalPositiveInteger(args, 'width', name);
      optionalPositiveInteger(args, 'height', name);
      optionalNonNegativeInteger(args, 'margin', name);
      optionalPositiveInteger(args, 'line_repeat', name);
      optionalString(args, 'palette', name);
      optionalPositiveInteger(args, 'max_pages', name);
      optionalNonNegativeNumber(args, 'pixels_per_token', name);
      if (args.palette !== undefined && args.palette !== 'bw' && args.palette !== 'row6') throw invalidParams(`${name} palette must be bw or row6.`);
      if (args.pixels_per_token !== undefined && args.pixels_per_token <= 0) throw invalidParams(`${name} pixels_per_token must be positive.`);
      return args;
    case 'enigma_delete':
...
export const toolDescriptors = [
  ...
  {
    name: 'enigma_visual_context',
    description: 'Render selected local Enigma context into deterministic PNG image pages for a vision-capable model. Images are plaintext-equivalent derived artifacts, not encryption; benchmark recall, downscaling, and billing for each model.',
    inputSchema: {
      type: 'object',
      properties: {
        bundlePath: { type: 'string' },
        ...
        palette: { type: 'string', enum: ['bw', 'row6'] },
...
const CORE_TOOL_NAMES = Object.freeze([
  ...
  'enigma_search',
  'enigma_context_pack',
  'enigma_visual_context',
  'enigma_delete',
  'enigma_verify_receipts',
]);

const CORE_TOOL_INPUT_SCHEMAS = Object.freeze({
  enigma_remember: Object.freeze({
    type: 'object',
    properties: {
      bundlePath: { type: 'string' },
      text: { type: 'string' },
  ...
  enigma_context_pack: Object.freeze({
    type: 'object',
    properties: {
      bundlePath: { type: 'string' },
      query: { type: 'string' },
  ...
  enigma_verify_receipts: Object.freeze({
    type: 'object',
    properties: {
      bundlePath: { type: 'string' },
    },
...
export const coreToolDescriptors = Object.freeze(CORE_TOOL_NAMES.map((name) => {
  const descriptor = toolDescriptors.find((tool) => tool.name === name);
  if (!descriptor) throw new Error(`Missing core MCP tool descriptor: ${name}`);
  return Object.freeze({
    ...descriptor,
    inputSchema: CORE_TOOL_INPUT_SCHEMAS[name] ?? descriptor.inputSchema,
  });
}));

export const toolDescriptorProfiles = Object.freeze({
  core: coreToolDescriptors,
  full: Object.freeze([...toolDescriptors]),
});

export function toolDescriptorsForProfile(profile = 'full') {
  const normalized = String(profile ?? 'full').trim().toLowerCase();
  const descriptors = toolDescriptorProfiles[normalized];
  if (!descriptors) throw new TypeError('MCP tool profile must be core or full');
  return descriptors;
}

export function createMcpToolProtocolAudit(descriptors = toolDescriptors) {
  return analyzeMcpToolProtocol(descriptors);
}
...
export async function enigma_visual_context(input = {}) {
  input = validateToolArguments('enigma_visual_context', input);
  const path = bundlePath(input);
  const { vault, passport } = await loadState(path);
  ...
  const pack = compileContextPack({
    ...
    passport,
    query,
    purpose: input.purpose ?? 'mcp_visual_context',
    optimize: input.optimize ?? String(query).trim().length > 0,
    max_estimated_tokens: input.max_estimated_tokens,
    limit: Number(input.limit ?? 8),
  });
  await persistState(path, vault);
  return createVisualContextCarrier({
    text: renderContextPackForModel(pack),
    width: input.width,
    height: input.height,
    margin: input.margin,
    line_repeat: input.line_repeat,
    palette: input.palette,
    max_pages: input.max_pages,
    pixels_per_token: input.pixels_per_token,
  });
}
...
function toolCallContent(result, toolName) {
  if (toolName === 'enigma_visual_context') {
    const manifest = visualContextCarrierManifest(result);
    return {
      content: [
...
async function handleSingleJsonRpcRequest(request, options = {}) {
  ...
  try {
    switch (request.method) {
      ...
      case 'tools/list':
        ...
        return notification ? undefined : jsonRpcResult(id, { tools: toolDescriptorsForProfile(options.toolProfile ?? options.tool_profile ?? 'full') });
      case 'resources/list':
      ...
      case 'tools/call': {
        ...
        const exposedTools = toolDescriptorsForProfile(options.toolProfile ?? options.tool_profile ?? 'full');
        if (!exposedTools.some((tool) => tool.name === params.name)) throw invalidParams('Unknown tool.');
...
export async function handleJsonRpcRequest(request, options = {}) {
  if (!Array.isArray(request)) return handleSingleJsonRpcRequest(request, options);
  if (request.length === 0) return jsonRpcError(null, JSON_RPC_ERROR.INVALID_REQUEST, 'Invalid Request');

  const responses = [];
  for (const item of request) {
    const response = await handleSingleJsonRpcRequest(item, options);
...
export function startStdioServer(io = {}) {
  ...
  const toolProfile = io.toolProfile ?? io.tool_profile ?? process.env.ENIGMA_MCP_TOOL_PROFILE ?? 'core';
  toolDescriptorsForProfile(toolProfile);
  ...
  async function processLine(line) {
    ...
    if (!trimmed) return;
    try {
      writeJsonLine(output, await handleJsonRpcRequest(JSON.parse(trimmed), { toolProfile }));
    } catch (error) {
