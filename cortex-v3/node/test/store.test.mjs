...
describe("EncryptedStore", () => {
  ...
  it("searches by key prefix", () => {
    ...
    assert.ok(
      matches.some((m) => m.key === "alice/1" && m.value.text === "a1")
    );
    assert.ok(
      matches.some((m) => m.key === "alice/2" && m.value.text === "a2")
    );
  });
  ...
  it("encrypts values on disk", () => {
    ...
    assert.ok(
      !raw.includes("plaintext value"),
      "plaintext must not appear in sqlite file"
    );
  });
  ...
  it("stores and retrieves embeddings", () => {
    ...
    assert.deepEqual(
      Array.from(all[0].vector).map((v) => Number(v.toFixed(6))),
      [0.1, 0.2, 0.3]
    );
  });
  ...
  it("encrypts embeddings on disk", () => {
    ...
    assert.ok(
      !raw.includes("1.1"),
      "embedding values must not appear in sqlite file"
    );
  });
});
