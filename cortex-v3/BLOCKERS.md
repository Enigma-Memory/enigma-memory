...
- 🔄 **Signed installers and code signing** — Azure Artifact Signing setup in progress; Microsoft account `noahkopp@outlook.com` confirmed in Edge and Azure portal opened, but manual password/MFA sign-in is required before automation can continue. Apple Developer enrollment still pending Apple ID credentials/2FA. Scaffolding at `docs/public-launch/code-signing-setup.md` and `.github/workflows/desktop-release.yml`.
## Notes
