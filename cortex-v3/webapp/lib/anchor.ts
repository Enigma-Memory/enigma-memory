...
export type SolanaWallet = {
  publicKey: PublicKey
  signTransaction: (tx: Parameters<AnchorProvider['wallet']['signTransaction']>[0]) => Promise<Parameters<AnchorProvider['wallet']['signTransaction']>[0]>
  signAllTransactions: (txs: Parameters<AnchorProvider['wallet']['signAllTransactions']>[0]) => Promise<Parameters<AnchorProvider['wallet']['signAllTransactions']>[0]>
}
...
export function getConnection(cluster: string = DEVNET_CLUSTER): Connection {
  return new Connection(clusterApiUrl(cluster as 'devnet' | 'testnet' | 'mainnet-beta'), DEFAULT_COMMITMENT)
}
...
export function getProvider(connection: Connection, wallet: SolanaWallet): AnchorProvider {
  return new AnchorProvider(connection, wallet as AnchorProvider['wallet'], {
    commitment: DEFAULT_COMMITMENT,
  })
}

export function getProgram<T extends Idl = Idl>(programName: ProgramName, provider: AnchorProvider): Program<T> {
  const idl = IDLS[programName] as T
  return new Program<T>(idl, provider)
}

export function useAnchorProgram(programName: ProgramName, wallet?: SolanaWallet, cluster?: string) {
  return useMemo(() => {
