...
export function renderMemoryDriveDashboard(state = createDesktopState()) {
  ...
  const updateStatus = normalizePublicStatus(state.desktopUpdate?.status, ['unknown', 'checking', 'current', 'available', 'blocked_unsigned', 'blocked_downgrade', 'blocked_channel', 'blocked_incomplete', 'blocked_version', 'installing', 'ready', 'error'], 'unknown');
  ...
  return {
    ...
    support_next_action: supportReport.support_next_action,
...
function renderSupportReportModel(diagnostics = {}) {
  ...
  return {
    ...
    support_next_action: ready
      ? supportReportNextAction({ id: 'copy_support_code', label: 'Copy support code', reason: 'Share the short report ID when support only needs a handoff code.' }, 'handoff_ready')
      : supportReportNextAction({ id: 'collect_support_report', label: 'Collect support report', reason: 'Run diagnostics before a support code exists.' }, 'report_needed')
  };
}

function supportReportNextAction(action, state) {
  return {
    schema: 'enigma.desktop.support_next_action.v1',
    state,
    ...action,
    primary_action: { id: action.id, label: action.label },
    claim_boundaries: {
...
function connectClientState(state, action) {
  if (state.mcp.status !== 'running') return withNotice(state, 'Start the local connector service before connecting an app.');
...
function updateDesktopUpdateStatusState(state, action) {
  ...
  return bump({
    ...
    desktopUpdate: {
      ...state.desktopUpdate,
      status: normalizePublicStatus(update.status, ['unknown', 'checking', 'current', 'available', 'blocked_unsigned', 'blocked_downgrade', 'blocked_channel', 'blocked_incomplete', 'blocked_version', 'installing', 'ready', 'error'], state.desktopUpdate.status),
...
function supportReportCodeText(report = {}) {
  return [
    'Enigma support code',
    `Report ID: ${safePublicString(report.report_id, 'not generated', 64)}`,
    `Status: ${safePublicString(report.status, 'not-run', 32)}`,
    `Checked: ${safePublicString(report.checked_at, 'not observed', 64)}`,
    'Privacy: support code contains public-safe status only; no raw memory, prompts, transcripts, provider responses, credentials, tokens, private keys, account identifiers, customer identifiers, raw logs, complete settings, or local paths.'
  ].join('\n');
}
...
function renderHomeScreen(screen) {
  ...
  const supportNextAction = dashboard.support_next_action;
  const supportNextButton = dashboard.support_report_ready && supportNextAction && supportNextAction.id !== dashboard.next_action.id
    ? `<button type="button" data-action="${escapeHtml(supportNextAction.id)}">${escapeHtml(supportNextAction.label)}</button>`
    : '';
  const supportNextNote = supportNextButton
    ? `<p><strong>Support handoff:</strong> ${escapeHtml(supportNextAction.reason || '')}</p>`
    : '';
  return renderScreen('home', screen.title, `
    <div class="two-column">
      <div class="panel status-panel">
        ...
        <div class="button-row"><button type="button" data-action="${escapeHtml(dashboard.next_action.id)}">${escapeHtml(dashboard.next_action.label)}</button>${supportNextButton}</div>
        ${supportNextNote}
      </div>
...
function renderSupportScreen(screen) {
  ...
  const supportNextAction = screen.support_next_action || screen.primary_action;
  const supportNextButton = supportNextAction.id !== screen.primary_action.id
    ? `<button type="button" data-action="${escapeHtml(supportNextAction.id)}">${escapeHtml(supportNextAction.label)}</button>`
    : '';
  return renderScreen('support', screen.title, `
    <div class="two-column">
      <div class="panel status-panel">
        ...
        <p><strong>Next support step:</strong> ${escapeHtml(supportNextAction.label)}. ${escapeHtml(supportNextAction.reason || '')}</p>
        <div class="button-row"><button type="button" data-action="${escapeHtml(screen.primary_action.id)}">${escapeHtml(screen.primary_action.label)}</button>${supportNextButton}</div>
      </div>
      <dl class="panel detail-list">
    ...
    <label class="panel">Copyable support code<textarea readonly>${escapeHtml(supportReportCodeText(screen))}</textarea></label>`);
}
  ...
  return renderScreen('mcp', screen.title, `
    <div class="two-column">
      <div class="panel status-panel">
        ...
        <p>This local connector service lets approved AI apps talk to Enigma on this device. Receipts and logs are the evidence.</p>
        <div class="button-row"><button type="button" data-action="start-mcp">Start local service</button><button type="button" data-action="stop-mcp">Stop local service</button></div>
      </div>
      <dl class="panel detail-list">
        ${renderMetric('Local endpoint', screen.endpoint)}
        ${renderMetric('Connection mode', screen.transport)}
...
function renderClientsScreen(screen) {
  ...
  const rows = screen.connected.length ? screen.connected.map((client) => `<li><strong>${escapeHtml(client.name)}</strong><span>${escapeHtml(client.status)} using local connector service</span></li>`).join('') : '<li class="empty-row">No clients connected.</li>';
  ...
  root.querySelectorAll('[data-action]').forEach((button) => {
    button.addEventListener('click', () => {
      ...
      switch (button.dataset.action) {
        ...
        case 'copy_support_code': {
          const report = renderSupportReportModel(state.desktopDiagnostics);
          if (typeof navigator !== 'undefined' && navigator.clipboard?.writeText) {
            navigator.clipboard.writeText(supportReportCodeText(report)).catch(() => undefined);
          }
          dispatch(selectDesktopScreen('support'));
          break;
        }
