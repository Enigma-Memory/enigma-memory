...
| `enigma next` | optional `--bundle` | Emits `enigma.next_action.v1`, a no-error next-step artifact. Missing bundles return a setup action; existing bundles reuse `first_run_status` to point at import/remember or app connection. Output redacts local paths and returns no raw memory. |
