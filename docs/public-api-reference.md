...
| `enigma_import_approve` | `approved` | `bundlePath`, `text`, `complete`, `now`, `confidence`, `reviewed` | Explicit local-vault import write. `approved:true` is required; previews that need review require `reviewed:true`. Success returns `enigma.import_approved_batch.v1` plus `enigma.import_batch_receipt.v1`; blocked writes return `enigma.import_approval_blocked.v1`. No raw memory text is returned. |
