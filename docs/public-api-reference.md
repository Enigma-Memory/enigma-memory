...
import { createMemoryOptimizationPlan, createVisualContextCarrier } from 'enigma-memory/optimizer';
...
| `enigma-memory/passport` | `./passport` | `packages/passport/src/index.js` | stable local/package | `createPassport`, `compileContextPack`, `renderContextPackForModel`, `verifyContextPack`, `createContextPassport`, `verifyContextPassport`, `createProofOfNonUse`, `verifyProofOfNonUse`, `createMemoryDriveHealthReport`. |
...
| `enigma-memory/mcp-server` | `./mcp-server` | `packages/mcp-server/src/index.js` | stable local/package | `toolDescriptors`, `resourceDescriptors`, `promptDescriptors`, `handlers`, memory tools, `enigma_visual_context`, `enigma_immune_ingress`, Memory Controller tools (`enigma_memory_weather`, `enigma_recall_veto`, `enigma_consent_grant`, `enigma_private_bubble`), `enigma_meter_usage`, settlement tools, `enigma_passport_summary_resource`, `enigma_standard_memory_prompt`, `createMcpToolProtocolAudit`, `createMcpImmuneIngressReport`, `assertMcpImmuneIngressPublicSafe`, `handleJsonRpcRequest`, `startStdioServer`, `default`. |
...
| `enigma-memory/optimizer` | `./optimizer` | `packages/optimizer/src/index.js` | stable local/package | `MEMORY_OPTIMIZATION_PRODUCT_THESIS`, `MEMORY_OPTIMIZATION_PLAN_SCHEMA`, `MEMORY_ACCESS_RECEIPT_SCHEMA`, visual carrier schema/thesis constants, `estimateTextTokens`, `estimateTokenCost`, `estimateVisualImageTokens`, `createMemoryOptimizationPlan`, `createMemoryAccessReceipt`, `createVisualContextCarrier`, `visualContextCarrierManifest`, `verifyVisualContextCarrier`, visual text encode/decode helpers, `assertNoRawMemoryOutput`. |
...
Visual context carriers are a separate private-output path: `renderContextPackForModel` creates the deterministic model-facing text and `createVisualContextCarrier` renders it into local PNG pages with hashes and a plaintext-free manifest. The image pages intentionally carry plaintext-equivalent context and therefore must not pass through `assertNoRawMemoryOutput` or be published as optimizer evidence. They are not encryption. Enigma does not claim universal OCR/model recall, input-token reduction, provider billing savings, or immunity to client-side downscaling; those are exact-model benchmark questions.
...
| `VISUAL_CONTEXT_CARRIER_SCHEMA_ID` / `VISUAL_CONTEXT_CARRIER_MANIFEST_SCHEMA` | Stable visual carrier manifest identifiers. | The JSON manifest is plaintext-free; referenced PNG buffers are private plaintext-equivalent artifacts. |
| `createVisualContextCarrier(input)` | Deterministically renders text into one or more X11 `6x10` PNG pages. | Recall and billing are not guaranteed; benchmark each model/client path. |
| `visualContextCarrierManifest(carrier)` | Returns the enumerable plaintext-free manifest. | Omits page PNG buffers and encoded source text. |
| `verifyVisualContextCarrier(carrier, options)` | Verifies PNG signatures/hashes and optional source hash/encoding round trip. | Verifies Enigma artifact integrity, not model OCR or semantic recall. |
| `estimateVisualImageTokens(input)` | Applies an explicit caller-supplied pixels-per-token formula. | No provider formula is assumed by default. |
