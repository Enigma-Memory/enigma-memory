...
`enigma next` is safe to run before or after setup. If no bundle exists, it returns `enigma.next_action.v1` with a `run_quickstart` action instead of a stack trace; after setup, it mirrors `first_run_status`.
