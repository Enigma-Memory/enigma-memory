...
5. Run [`scripts/run-public-beta-qa-matrix.mjs`](../../scripts/run-public-beta-qa-matrix.mjs) and generate the public-safe beta evidence packet with artifact hashes, signing/notarization status, update verification, `BETA-*` scenario results from the [required scenario catalog](qa-support-observability.md#required-scenario-catalog), known limitations, support owner, release owner, signing owner, and claim-boundary review.
...
Public beta holds on any one of these: default user needs terminal/npm/Node/manual JSON, installer trust requires bypass-first guidance for the target beta audience, diagnostics/proof/support/export leaks forbidden fields, update can corrupt app/vault or crosses channels, connector write lacks consent/backup/rollback, public copy overclaims, automated matrix status is `fail`, `blocked`, `missing`, or `pending`, PR approval/merge is absent, `0.1.19` is unpublished, signed Windows/macOS artifacts are missing, Apple/Microsoft signing identity or signing-secret custody evidence is missing, code-signing/notarization evidence is missing, reviewer approval is absent, clean-machine manual QA is incomplete, support dry run is incomplete, or any of those items is claimed complete without a supporting public-safe repo artifact/evidence ref.
