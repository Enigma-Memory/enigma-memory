...
- Public beta QA can now ingest public-safe clean-machine smoke artifacts with `npm run public-beta-qa -- --clean-machine-smoke <smoke.json>`, per-scenario support dry-run summaries with repeated `--support-dry-run <summary.json>` flags, and registry install evidence with `--registry-install <registry.json>`; signed installers, release approval, and release-packet blockers still remain until their own evidence exists.
