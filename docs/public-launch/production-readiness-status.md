...
- Public beta QA can now ingest a public-safe clean-machine smoke artifact with `npm run public-beta-qa -- --clean-machine-smoke <smoke.json>`; signed installers, npm publication, release approval, support dry-run, and release-packet blockers still remain until their own evidence exists.
