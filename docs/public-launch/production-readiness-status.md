...
- Static `website/` pages: home, download, setup, help hub, install/connect/troubleshooting guides, privacy, proofs, FAQ, launch status, and developer CLI appendix. Home/download/install pages now gate the consumer path on signed-build readiness, present the local trust boundary, Import Sandbox, Proof Activity, and unsigned-build caveat without linking consumers to internal release-ops docs or claiming signed public release.
...
- Public beta QA can now ingest public-safe clean-machine smoke artifacts with `npm run public-beta-qa -- --clean-machine-smoke <smoke.json>`, per-scenario support dry-run summaries with repeated `--support-dry-run <summary.json>` flags, registry install evidence with `--registry-install <registry.json>`, desktop release evidence with `--desktop-release-evidence <desktop.json>`, and production handoff packets with `--production-handoff-packet <handoff.json>`; PR approval/reviewer gates still remain until their own evidence exists.
