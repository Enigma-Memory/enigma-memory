...
- Release audit now passes the direct Node test runner POSIX-style `test/*.test.mjs` paths so the audited `npm-test` gate matches package `npm test` behavior across Windows runners without nesting npm lifecycle scripts.
