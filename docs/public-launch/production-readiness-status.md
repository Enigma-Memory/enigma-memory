...
- Release owners can also run `node scripts/run-public-beta-qa-matrix.mjs --plain` for a one-screen Advisor hold/ship summary, `node scripts/run-clean-machine-smoke.mjs --plain --out <smoke.json>` for readable clean-machine QA stdout while preserving JSON evidence, and `node scripts/build-support-dry-run-summary.mjs ... --plain --out <summary.json>` for readable support dry-run evidence.
...
- `npm test` at repo root: **631/631 pass**.
