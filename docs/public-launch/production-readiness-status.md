...
- Release owners can also run `node scripts/run-public-beta-qa-matrix.mjs --plain` for a one-screen Advisor hold/ship summary with ranked next actions and no local-path or private-evidence output.
...
- `npm test` at repo root: **627/627 pass**.
