...
- First-run `status`, `next`, and `doctor` guidance now defaults to non-destructive quickstart/remember/connect dry-run commands; status JSON also keeps connector setup as a dry-run preview so consumers do not see stale or mutating verification guidance.
...
- `npm run public-beta:review` at repo root: **pass**; writes public-safe evidence manifest, generated support dry-run summaries, and QA matrix artifacts while Advisor decision remains **HOLD**.
