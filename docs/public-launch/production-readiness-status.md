...
- Release owners can also run `node scripts/run-public-beta-qa-matrix.mjs --plain` for a one-screen Advisor hold/ship summary with ranked next actions, and `node scripts/run-clean-machine-smoke.mjs --plain --out <smoke.json>` for readable clean-machine QA stdout while preserving JSON evidence.
...
- `npm test` at repo root: **629/629 pass**.
