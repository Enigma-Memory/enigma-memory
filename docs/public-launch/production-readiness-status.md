...
- Desktop Tauri metadata and update-check UI are aligned to public beta package version `0.1.19`.
...
- Desktop Claude cards now make the Claude `.mcpb` extension handoff the primary non-writing path, label JSON config preview as advanced fallback, and show remove/disable plus config-fallback test boundaries.
...
- README developer CLI quickstart now uses `enigma quickstart` plus connector `--dry-run` preview instead of a default auto-connect overwrite command.
- Updated `docs/install-anywhere.md` to use quickstart plus connector dry-run preview instead of auto-connect overwrite defaults.
- Updated `docs/client-connectors.md` to make one-client dry-run previews the connector default and remove auto-connect overwrite commands from the clear path.
- Updated browser extension, native host, and MCP package docs so extension setup uses quickstart plus connector dry-run preview before any connector write.
- Updated `docs/developer-ecosystem.md` so developer copy starts from quickstart and connector dry-run previews instead of auto-connect overwrite setup.
- Updated `docs/public-api-reference.md` to show quickstart without overwrite and connector dry-run preview in published-package examples.
- CLI root help and install summaries now start from non-overwrite quickstart/setup commands and dry-run connector previews instead of default overwrite copy.
- Generated installer source assets now mirror the safe first-run path: test drive, quickstart with the selected bundle, doctor with the same bundle, then bundled connector dry-run preview.
...
- Public FAQ now sends "Memory Drive not ready" users to the consumer Memory Drive recovery guide first instead of the Developer CLI appendix.
...
- First-run collisions now keep `--plain` output human-readable and path-redacted; setup, quickstart, and test-drive make a different empty output folder the primary recovery instead of putting `--overwrite` in the `Next:` line.
- First-run `status`, `next`, `doctor`, setup help, and connector wizard guidance now defaults to non-destructive quickstart/remember/connect dry-run commands; one-command connector recommendations install, create the local Memory Drive, then preview the app connection instead of writing app settings.
...
- Release owners can run `npm run public-beta:status` for the shortest plain-language HOLD/pass/blocker view. `npm run public-beta:review` remains the fuller local command: it writes `.enigma/public-beta/evidence-manifest.json`, writes generated public-safe support dry-run summaries for diagnostics and crash triage, writes `.enigma/public-beta/qa-matrix.json`, prints the bounded Advisor summary, and points to `npm run public-beta:evidence-templates -- --out-dir .enigma/public-beta --plain` for non-destructive starter evidence files while treating external evidence files as blockers instead of script errors. The Advisor now prints `Collect next:` lines that name the evidence item, target file under the selected review/template output directory, and public-safe fields to collect.
- Release owners can generate the full starter set with `npm run public-beta:evidence-templates -- --out-dir .enigma/public-beta --plain`: clean-machine smoke template, clean-machine smoke plan, diagnostics support dry-run template, crash support dry-run template, registry install template, desktop release evidence template, production handoff template, and evidence manifest. They can then run `npm run public-beta:advisor -- --evidence-manifest .enigma/public-beta/evidence-manifest.json` for the one-screen Advisor hold/ship summary; missing or template-only evidence paths stay blockers instead of script errors, and the manifest itself prints the same collection checklist for clean-machine smoke, support dry-runs, registry install, desktop release evidence, and production handoff packet.
- Release audit now passes the direct Node test runner POSIX-style `test/*.test.mjs` paths and serializes file execution on Windows so the audited `npm-test` gate avoids child-process temp-path flakes without nesting npm lifecycle scripts.
- Release owners can also run `node scripts/run-public-beta-qa-matrix.mjs --plain` directly, `node scripts/run-clean-machine-smoke.mjs --plain --out <smoke.json>` for readable clean-machine QA stdout, `node scripts/build-support-dry-run-summary.mjs --preset diagnostics --triage-result <result> --bundle-privacy-check-status <status> --plain --out <summary.json>` for readable support dry-run evidence without retyping support scenario IDs, `node scripts/release-evidence-desktop.mjs --plain` for readable desktop release evidence, `npm run cloudflare:pages:packet -- --plain ...` for readable Cloudflare Pages release-packet output, `npm run cloudflare:token-policy -- --plain ...` for readable least-privilege Cloudflare token policy output, `npm run cloudflare:token-request -- --plain ...` for readable Cloudflare token request-body output, `npm run production:manifest -- --plain ...` for readable infrastructure readiness output, and `npm run production:handoff -- --plain ...` for readable production handoff output. These plain modes still keep public beta on hold until required external evidence and approvals exist.
- `npm run production:status -- --plain ...` now also prints `Collect next:`, `Record in:`, and `Verify with:` lines for the current release-owner blocker, starting with Cloudflare credential presence evidence and continuing through Worker permission, hosted backend refs, operator acceptance, and final release verification.
