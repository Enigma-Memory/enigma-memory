...
- `blocked` means an external/manual prerequisite is not satisfied, such as PR approval/merge, `0.1.19` npm publish evidence, Apple/Microsoft signing identity readiness, verified Windows signature evidence, verified macOS signature/notarization/stapling evidence, update rollback evidence, reviewer approval, clean-machine manual QA, or support dry run.
...
The automated matrix is a hold/block reporter, not a public readiness substitute. The current code may pass local automated/static slices and still be blocked for public beta, production, or GA when external evidence is absent. The matrix cannot prove clean Windows/macOS installs, real code-signing/notarization/stapling, rollback rehearsals, `0.1.19` npm publication, merged release approval, Apple/Microsoft signing identities or secret custody, reviewer approval, support readiness, or production installer trust. Do not mark signed installer, npm publish, merge, notarization/stapling, update rollback, or clean-machine manual QA complete unless the supporting public-safe artifact/evidence exists in the repo. Public beta remains held unless the automated report, the manual install matrix, release-owner checklist, support dry run, and public-safe release packet are all green.

When a blocker can be narrowed to public-safe next artifacts, the runner may emit `missing_evidence_items` without changing the blocker status. Create starter files with `npm run public-beta:evidence-templates -- --out-dir .enigma/public-beta --plain` and keep them as blockers until real evidence replaces every placeholder. The current concrete support item is `EV-P10-SUPPORT-DRY-RUN-SUMMARY` on `BLOCKER-SUPPORT-DRY-RUN`: collect one diagnostics summary at `.enigma/public-beta/support-dry-run-diagnostics.json` and one crash summary at `.enigma/public-beta/support-dry-run-crash.json`. Build the reviewable diagnostics summary with `npm run production:support-dry-run -- --preset diagnostics --triage-result needs_user_action --bundle-privacy-check-status pass --support-artifact .enigma/redacted-support-summary.json --out .enigma/public-beta/support-dry-run-diagnostics.json` when a redacted `enigma.support_summary.v1` or `enigma.diagnostics.v1` artifact exists; use `--preset crash --out .enigma/public-beta/support-dry-run-crash.json` for `BETA-CRASH-001`. The evidence must include `privacy_scan.status=pass` with zero private findings before the support blocker clears. Rerunning `npm run public-beta:review` preserves existing real support summaries, refreshes stale generated placeholders that predate privacy-scan metadata, and generates only missing blocker placeholders.

The matrix also emits `next_actions`: a ranked, public-safe release-owner queue that maps active blockers to owner refs, scenario IDs, evidence refs, missing evidence items, and `Collect next:` file targets. This is a triage aid only; an action remains blocked until the referenced public-safe evidence is actually collected.
