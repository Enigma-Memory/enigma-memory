...
If the matrix reports `BLOCKER-SUPPORT-DRY-RUN`, the concrete public-safe evidence item to collect is `EV-P10-SUPPORT-DRY-RUN-SUMMARY`: a support dry-run summary with scenario ID, issue code, triage result, bundle privacy-check status, and support owner ref. Generate it with `npm run production:support-dry-run -- --scenario-id BETA-DIAG-001 --issue-code DIAG-BUNDLE-PREVIEWED --triage-result needs_user_action --bundle-privacy-check-status pass --support-owner-ref ref:role:beta-support --out .enigma/support-dry-run-summary.json`. Recording that item makes the missing evidence reviewable; it does not clear clean-machine, signing, release, approval, or support blockers by itself.
