...
If the matrix reports `BLOCKER-SUPPORT-DRY-RUN`, the concrete public-safe evidence item to collect is `EV-P10-SUPPORT-DRY-RUN-SUMMARY`: a support dry-run summary with scenario ID, issue code, triage result, bundle privacy-check status, support owner ref, and optionally an allowlisted hash snapshot of a redacted `enigma.support_summary.v1` or `enigma.diagnostics.v1` artifact. Generate starter placeholders with `npm run public-beta:evidence-templates -- --out-dir .enigma/public-beta --plain`; replace `.enigma/public-beta/support-dry-run-diagnostics.json` with `npm run production:support-dry-run -- --preset diagnostics --triage-result needs_user_action --bundle-privacy-check-status pass --support-artifact .enigma/redacted-support-summary.json --out .enigma/public-beta/support-dry-run-diagnostics.json` when a redacted artifact exists, and replace `.enigma/public-beta/support-dry-run-crash.json` with the same command using `--preset crash --out .enigma/public-beta/support-dry-run-crash.json`. Presets fill only scenario, issue, and support-owner defaults; they never set triage result or bundle privacy status. Rerunning `npm run public-beta:review` preserves existing support summaries and generates only missing blocker placeholders. Recording those items makes the missing evidence reviewable; it does not mark either scenario pass without the actual public-safe artifacts.
