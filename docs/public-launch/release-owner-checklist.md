...
Run `npm run public-beta:status` for the fastest plain-language HOLD/pass/blocker view. Run `npm run public-beta-qa` or `node scripts/run-public-beta-qa-matrix.mjs --json` when a public-safe JSON matrix artifact is needed before asking Advisor for beta approval. The runner lives at [`scripts/run-public-beta-qa-matrix.mjs`](../../scripts/run-public-beta-qa-matrix.mjs) and uses the [required scenario catalog](qa-support-observability.md#required-scenario-catalog): `BETA-INSTALL-001`, `BETA-FIRST-001`, `BETA-CLIENT-CLAUDE-001`, `BETA-CLIENT-001`, `BETA-CLIENT-002`, `BETA-CLIENT-003`, `BETA-PROOF-001`, `BETA-OFFLINE-001`, `BETA-CONFIG-001`, `BETA-CONFIG-002`, `BETA-DIAG-001`, `BETA-CRASH-001`, `BETA-SIGNING-WINDOWS-001`, `BETA-SIGNING-MACOS-001`, `BETA-UPDATE-001`, `BETA-NPM-001`, and `BETA-MERGE-001`.
...
If the matrix reports `BLOCKER-SUPPORT-DRY-RUN`, the concrete public-safe evidence item to collect is `EV-P10-SUPPORT-DRY-RUN-SUMMARY`: a support dry-run summary with scenario ID, issue code, triage result, bundle privacy-check status, support owner ref, privacy-scan status with zero private findings, and optionally an allowlisted hash snapshot of a redacted `enigma.support_summary.v1` or `enigma.diagnostics.v1` artifact. Generate starter placeholders with `npm run public-beta:evidence-templates -- --out-dir .enigma/public-beta --plain`; replace them with real evidence, not template files. Replace `.enigma/public-beta/support-dry-run-diagnostics.json` with `npm run production:support-dry-run -- --preset diagnostics --triage-result needs_user_action --bundle-privacy-check-status pass --support-artifact .enigma/redacted-support-summary.json --out .enigma/public-beta/support-dry-run-diagnostics.json --json`, and replace `.enigma/public-beta/support-dry-run-crash.json` with the same command using `--preset crash` after the crash support drill is observed. `npm run public-beta:review` preserves existing support dry-run evidence and only creates missing placeholders.
