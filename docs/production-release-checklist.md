...
| Hosted customer API key lifecycle readiness | API key lifecycle contract surface expected / provider issuance blocked | `production:hosted-api-key` builds readiness evidence for customer API key issue, rotate, revoke, and audit operations only. The packet keeps `customer_api_keys_live:false` unless all required operation evidence refs and an operator approval ref are present. It is not live key issuance, key rotation, provider revocation, KMS custody proof, provider payload proof, or customer-secret evidence. |
...
- [ ] Hosted-cloud contract builders and validators are present for user account, tenant, hosted vault, API key, API key lifecycle packet, usage billing record, dashboard summary, backup drill, incident/SLA refs, and the customer lifecycle packet; validators reject raw memory, prompts, provider responses, credentials, API key secret material, financial outcome claims, provider-side deletion/model-forgetting claims, missing lifecycle surfaces, missing API key operation surfaces, and missing operator evidence refs.
- [ ] API key lifecycle packet evidence is generated with `npm run production:hosted-api-key -- --tenant <id> --subject <opaque-subject-ref> --operation <issue|rotate|revoke|audit> --out ./.enigma/hosted-api-key-lifecycle.json`; missing or blocked `--evidence-ref <phase=status:ref>` values for the selected operation and missing `--operator-approval-ref <ref>` must keep `customer_api_keys_live:false`.
- [ ] API key lifecycle packets are treated as readiness evidence validation only, not customer API key issuance; they do not create raw keys, store key material, call auth providers, call KMS, rotate/revoke provider credentials, export provider payloads, or prove provider-side deletion/model forgetting.
