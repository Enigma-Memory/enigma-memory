...
test('desktop public dashboard omits raw memory, prompts, transcripts, tokens, paths, and provider responses', async () => {
  ...
  const model = renderDesktopModel(state);
  const dashboard = renderMemoryDriveDashboard(state);
  const blockedUpdate = renderMemoryDriveDashboard(desktopReducer(state, updateDesktopUpdateStatus({
    status: 'blocked_unsigned',
    version: '1.0.1',
    issue_codes: ['UNSIGNED_UPDATE_BLOCKED'],
  }, { now: '2026-06-28T00:09:00.000Z' })));
  assert.equal(blockedUpdate.update_status, 'blocked_unsigned');
  assert.ok(blockedUpdate.issue_codes.includes('UNSIGNED_UPDATE_BLOCKED'));
  const downgradeUpdate = renderMemoryDriveDashboard(desktopReducer(state, updateDesktopUpdateStatus({
    status: 'blocked_downgrade',
    version: '0.9.9',
    issue_codes: ['DOWNGRADE_UPDATE_BLOCKED'],
  ...
  const incompleteUpdate = renderMemoryDriveDashboard(desktopReducer(state, updateDesktopUpdateStatus({
    status: 'blocked_incomplete',
    version: '1.0.1',
    issue_codes: ['INCOMPLETE_UPDATE_BLOCKED'],
  }, { now: '2026-06-28T00:12:00.000Z' })));
  assert.equal(incompleteUpdate.update_status, 'blocked_incomplete');
  assert.ok(incompleteUpdate.issue_codes.includes('INCOMPLETE_UPDATE_BLOCKED'));
...
test('desktop Tauri dashboard exposes Memory Controller and Import Sandbox consumer controls', async () => {
  ...
  assert.match(wizard, /Unsigned update blocked/);
...
test('public website explains consumer install path without unsupported claims', async () => {
  ...
  assert.match(publicLaunchDocs, /get a Memory Drive/);
  assert.match(publicLaunchDocs, /One Memory Drive/);
  ...
  assert.match(developerCli, /Enigma Memory Drive bundle/);
  assert.match(developerCli, /Enigma-controlled Memory Drive and receipt events/);
  assert.doesNotMatch(developerCli, /Enigma vault bundle|Enigma-controlled local vault/);
  ...
  assert.match(installAnywhere, /Enigma Memory Drive bundle/);
  assert.match(installAnywhere, /local Enigma Memory Drive/);
  assert.match(installAnywhere, /Memory Drive bundles, private keys/);
  assert.doesNotMatch(installAnywhere, /Enigma vault bundle|local Enigma vault|Enigma-controlled vault state|bake vault bundles/i);
