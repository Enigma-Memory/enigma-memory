...
test('production status board summarizes current launch posture', () => {
  ...
  assert.equal(board.release_owner_next_collection.evidence_item_id, 'EV-PROD-CLOUDFLARE-CREDENTIALS');
  assert.equal(board.release_owner_next_collection.target_file, '.enigma/cloudflare-credentials-current.json');
  assert.match(board.release_owner_next_collection.collect, /out-of-band/);
...
test('production status board plain output is readable and claim-bounded', () => {
  ...
  assert.match(plain, /Collect next: EV-PROD-CLOUDFLARE-CREDENTIALS/);
  assert.match(plain, /Record in: \.enigma\/cloudflare-credentials-current\.json/);
  assert.match(plain, /Verify with: npm run production:cloudflare-credentials -- --out \.enigma\/cloudflare-credentials-current\.json/);
