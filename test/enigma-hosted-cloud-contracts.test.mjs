...
test('hosted cloud validators reject raw memory, prompts, provider responses, and credentials', () => {
  ...
  assert.throws(() => validateUserAccountContract({ ...account, notes: 'Bearer abcdefghijklmnopqrstuvwxyz' }), /credential-looking/);
});
...
test('hosted cloud validators cover dashboard, backup drill, incident refs, and api key contracts', () => {
  ...
  assert.throws(() => validateApiKeyContract({ ...apiKey, api_key_value: 'sk-abcdefghijklmnopqrstuvwxyz' }), /not allowed/);
